...
//int init_subr(char *name, int type, struct obj *(*fcn)());
...
struct obj *consp(struct obj *x);
struct obj *car(struct obj *x);
struct obj *cdr(struct obj *x);
struct obj *setcar(struct obj *cell, struct obj *value);
struct obj *setcdr(struct obj *cell, struct obj *value);
struct obj *flocons(double x);
struct obj *numberp(struct obj *x);
struct obj *plus(struct obj *x, struct obj *y);
struct obj *ltimes(struct obj *x, struct obj *y);
struct obj *difference(struct obj *x, struct obj *y);
struct obj *quotient(struct obj *x, struct obj *y);
struct obj *greaterp(struct obj *x, struct obj *y);
struct obj *lessp(struct obj *x, struct obj *y);
struct obj *eq(struct obj *x, struct obj *y);
struct obj *eql(struct obj *x, struct obj *y);
struct obj *symcons(char *pname, struct obj *vcell);

struct obj *symbol_boundp(struct obj *x, struct obj *env);
struct obj *symbol_value(struct obj *x, struct obj *env);
struct obj *cintern_soft(char *name);
struct obj *cintern(char *name);
char *must_malloc(unsigned long size);
struct obj *rintern(char *name);
struct obj *subrcons(int type, char *name, struct obj *(*f)(void));
...
struct obj *get_newspace(void);
void scan_newspace(struct obj *newspace);
void gc(void);
struct obj *gc_status(struct obj *args);
struct obj *leval_args(struct obj *l, struct obj *env);
struct obj *extend_env(struct obj *actuals, struct obj *formals, struct obj *env);
struct obj *envlookup(struct obj *var, struct obj *env);
struct obj *leval(struct obj *x, struct obj *env);
struct obj *setvar(struct obj *var, struct obj *val, struct obj *env);
struct obj *leval_setq(struct obj *args, struct obj *env);
struct obj *syntax_define(struct obj *args);
struct obj *leval_define(struct obj *args, struct obj *env);
struct obj *leval_if(struct obj **pform, struct obj **penv);
struct obj *leval_lambda(struct obj *args, struct obj *env);
struct obj *leval_progn(struct obj **pform, struct obj **penv);
struct obj *leval_or(struct obj **pform, struct obj **penv);
struct obj *leval_and(struct obj **pform, struct obj **penv);
struct obj *leval_let(struct obj **pform, struct obj **penv);
struct obj *reverse(struct obj *l);
struct obj *let_macro(struct obj *form);
struct obj *leval_quote(struct obj *args, struct obj *env);
struct obj *leval_tenv(struct obj *args, struct obj *env);
struct obj *lread(void);
struct obj *lreadtk(int j);
struct obj *copy_list(struct obj *x);
struct obj *oblistfn(void);
void close_open_files(void);
struct obj *vload(char *fname);
struct obj *load(struct obj *fname);
struct obj *quit(void);
struct obj *nullp(struct obj *x);
struct obj *arglchk(struct obj *x);
struct obj *lprint(struct obj *exp);
struct obj *lreadf(FILE *f);
struct obj *lreadr(FILE *f);
void process_cla(int argc, char **argv);
int flush_ws(FILE *f,char *eoferr);
struct obj *lreadparen(FILE *f);
struct obj *lprin1(struct obj *exp);
struct obj *lerr(struct obj *message,struct obj*x);
void init_subr(char *name, int type, struct obj *(*fcn)());
...
{    
     ...
     fprintf(stdout," dump of heap \n");
     for(l=heap_org; l < heap; ++l)
     {
          ...
          switch TYPE(l)
          {
               ...
               case tc_fsubr:
               case tc_msubr:
                    fprintf(stdout,"#<SUBR(%d) %s>\n",TYPE(l),(*l).storage_as.subr.name);
               break;
               case tc_closure:
                    fprintf(stdout,"#<CLOSURE ");
                    lprin1(car((*l).storage_as.closure.code));
                    fprintf(stdout," ");
                    lprin1(cdr((*l).storage_as.closure.code));
                    fprintf(stdout,">\n");
                    break;
          }
          fflush(stdout);
...
void err(char *message, struct obj *x)
{
    nointerrupt = 1;
    if NNULLP(x) 
        fprintf(stdout,"ERROR: %s (see errobj)\n",message);
    else fprintf(stdout,"ERROR: %s\n",message);
    if (errjmp_ok == 1) {setvar(sym_errobj,x,NIL); longjmp(errjmp,1);}
    fprintf(stdout,"FATAL ERROR DURING STARTUP OR CRITICAL CODE SECTION\n");
    exit(1);
}

struct obj *lerr(struct obj *message,struct obj*x) 
{
    if NTYPEP(message,tc_symbol) 
        err("argument to error not a symbol",message);
    err(PNAME(message),x);
    return(NIL);
}
...
struct obj * plus(struct obj *x,struct obj *y)
{
     if NTYPEP(x,tc_flonum) 
          err("wta(1st) to plus",x);
     if NTYPEP(y,tc_flonum) 
...
void init_storage(void)
{
     ...
     memset(heap_1, 0, sizeof(struct obj)*heap_size);
     memset(heap_2, 0, sizeof(struct obj)*heap_size);
     heap = heap_1;
     which_heap = 1;
     heap_org = heap;
...
struct obj * gc_relocate(struct obj *x)
{
    ...
    {
        case tc_flonum:
      newobj = flocons(FLONM(x));
      break;
    case tc_cons:
      newobj = cons(CAR(x),CDR(x));
      break;
    case tc_symbol:
      newobj = symcons(PNAME(x),VCELL(x));
      break;
    case tc_closure:
      newobj = closure((*x).storage_as.closure.env,
     ...
     switch TYPE(x)
     {
          ...
          case tc_cons:
               ...
               switch TYPE(tmp)
	          {
	               ...
	               case tc_subr_2:
	                    return(SUBRF(tmp)(leval(car(CDR(x)),env),leval(car(cdr(CDR(x))),env)));
	               case tc_subr_3:
	                    return(SUBRF(tmp)(leval(car(CDR(x)),env),leval(car(cdr(CDR(x))),env),
			     leval(car(cdr(cdr(CDR(x)))),env)));
	               ...
	               case tc_msubr:
	                    ...
	                    goto loop;
	               case tc_closure:
	                    env = extend_env(leval_args(CDR(x),env),car((*tmp).storage_as.closure.code),(*tmp).storage_as.closure.env);
...
struct obj *leval_define(struct obj *args,struct obj *env)
{
  ...
  var = car(tmp);
  if NTYPEP
    (var, tc_symbol) err("wta(non-symbol) to define", var);
...
struct obj *leval_progn(struct obj **pform,struct obj **penv)
{
	...
	while(NNULLP(next)) 
	{
		leval(car(l),env);l=next;next=cdr(next);
	}
	*pform = car(l); 
	return(truth);
}
...
struct obj *lprin1(struct obj *exp)
{
     ...
     switch TYPE(exp)
     {
          ...
          case tc_cons:
	          ...
	          {
                    fprintf(stdout," ");lprin1(car(tmp));
               }
               if NNULLP(tmp) {fprintf(stdout," . ");lprin1(tmp);}
               fprintf(stdout,")");
               break;
          case tc_flonum:
               fprintf(stdout,"%g",FLONM(exp));
          break;
          case tc_symbol:
               fprintf(stdout,"%s",PNAME(exp));
          break;
          case tc_subr_0:
          case tc_subr_1:
          case tc_subr_2:
          ...
          case tc_fsubr:
          case tc_msubr:
               fprintf(stdout,"#<SUBR(%d) %s>",TYPE(exp),(*exp).storage_as.subr.name);
          break;
          case tc_closure:
               fprintf(stdout,"#<CLOSURE ");
...
struct obj *load(struct obj *fname)
{
    if NTYPEP(fname,tc_symbol) 
        err("filename not a symbol",fname);
